    ...
    def load_restaurants(self, restaurant_json, db):
        ...
        self.stdout.write(
            self.style.MIGRATE_HEADING(
                f"Embedding restaurant data in {segment_count} segments..."
            )
        )

        inserted_restaurants = services.embed_restaurant_data(
            segment_collection, restaurant_json, command=self
        )
