...
class Command(BaseCommand):
    ...
    def load_segments(self, segment_json):
        ...
        segment_restaurant_data = (
            set(i) for i in ijson.items(segment_json, "item.restaurants")
        )
        ...
        self.stdout.write(
            self.style.MIGRATE_HEADING(
                f"Creating segments - restaurants relationships..."
            )
        )
        with tqdm(total=len(segment_data)) as progress_bar:
            for index, uid_set in enumerate(segment_restaurant_data):
                segment_restaurant_objects = Restaurant.objects.filter(
                    uidentifier__in=uid_set
                )
