...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):        
        try:
            ...
            self.stdout.write(self.style.MIGRATE_HEADING(f"Deserializing MongoDB data..."))
            data = [s for s in collection.find({})]
            
            self.stdout.write(self.style.MIGRATE_HEADING(f"Migrating data and creating relationships..."))
            ...
            with tqdm(total=len(data)) as progress_bar:
                for segment in data:
                    ...
                    restaurant_objects = set(Restaurant(**r) for r in segment["restaurants"])
