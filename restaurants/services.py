...
def get_mongo_client():
    try:
        ...
        client = MongoClient(
            ...
            serverSelectionTimeoutMS=CLIENT_TIMEOUT_SECONDS,
        )
        return client
...
def embed_restaurant_data(segment_collection, restaurant_json, command=None):
    ...
    with tqdm(total=segment_count) as progress_bar:
        for segment in segment_collection.find():
            ...
            uids = set(r["uidentifier"] for r in restaurant_data).intersection(
                segment_restaurant_uids
            )
            restaurant_data = generator_from_json(restaurant_json)
            ...
            segment_collection.update_one(
                {"_id": segment["_id"],}, {"$set": {"restaurants": insert_list},}
            )
    ...
    if command:
        command.stdout.write(
            command.style.SUCCESS(f"Embedded {inserted_restaurants} restaurant objects")
        )
